...
OstrichStorePrototype.append = function (version, triples, callback, self) {
  ...
  this._append(version, triples, function (error, insertedCount) {
    ...
    // Call the append-callbacks if no appends are going on anymore.
    if (!this_._appends) {
      for (var i = 0; i < this_._appendCallbacks.length; i++)
        this_._appendCallbacks[i]();
    }
  }, self);
};
...
OstrichStorePrototype.close = function (remove, callback, self) {
  ...
  if (!this._appends)
    this._close(remove, callback, self);
  else {
    var this_ = this;
    this._appendCallbacks.push(function () {
      this_._close(remove, callback, self);
    });
  }
};
